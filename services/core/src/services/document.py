    ...
    async def create_document(self, db: AsyncSession, upload_data: DocumentUpload):
        ...
        try:
            ...
            try:
                ...
                logger.critical(f"CRITICAL ORPHAN FILE: Failed to clean up S3 file {s3_key}. Error: {cleanup_error}")
            
            raise HTTPException(status_code=500, detail="Failed to save document metadata. Upload reverted.")
